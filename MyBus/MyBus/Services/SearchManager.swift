...
open class SearchManager: NSObject {
    var currentSearch: BusSearchResult?

    // MARK: - Instantiation
    open class var sharedInstance: SearchManager {
        return _sharedInstance
    }

    override init() { }

    func getBusLines(_ origin: CLLocationCoordinate2D, destination: CLLocationCoordinate2D, completionHandler: @escaping ([BusRouteResult]?, Error?) -> ()) -> Void {
     ...
     */
    func search(_ origin: String, destination: String, completionHandler: @escaping (BusSearchResult?, Error?) -> ()) -> Void {
     ...
     */
    func search(_ origin: RoutePoint, destination: RoutePoint, completionHandler: @escaping (BusSearchResult?, Error?) -> ()) -> Void {
    ...
    /**
     ...
     :returns: Completion callback with an optional RoadResult and Error?
     */
    func getRoad(_ busRouteResult: BusRouteResult, completionHandler: @escaping (RoadResult?, Error?)->()) -> Void {
     ...
     */
    func getWalkingRoutes(_ roadResult: RoadResult, completion: @escaping ()->()) -> Void {
    ...
    func getCompleteRoute(_ idBusLine: Int, busLineName: String, completion: @escaping (CompleteBusRoute?, Error?)->()) -> Void {
        ...
        if let busItinerary = busItineraryResult {
            let secondsInADay: TimeInterval = 3600 * 30
